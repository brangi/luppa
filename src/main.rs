...
use luppa::{
    PassportValidator,
    models::{ValidationResult, MrzData, VisualData, ValidationIssueType}
};
...
fn main() {
    ...
    match validator.validate(image_path) {
        Ok(result) => {
            let mrz_data = luppa::processing::OcrProcessor::extract_mrz(&luppa::processing::ImageProcessor::process_image(image_path).unwrap()).unwrap();
            let visual_data = luppa::processing::OcrProcessor::extract_visual_data(&luppa::processing::ImageProcessor::process_image(image_path).unwrap()).unwrap();
